...
public class ModelGenerator {
    ...
    public static <T> T getModelByMapping(Response response, Class<T> clazz) {
        try {
            return new ObjectMapper().readValue(response.asString(), clazz);
        } catch (Exception e) {
            e.printStackTrace();
            throw new NoSuchElementException();
        }
    ...
    public static <T> T getModelByMappingLink(String link, Class<T> clazz) {
        try {
            return new ObjectMapper().readValue(new JsonApi(link).executeRequest().asString(), clazz);
        } catch (Exception e) {
            e.printStackTrace();
            throw new NoSuchElementException();
        }
    ...
    public static <T> T getModelByMappingWithKey(RequestEnums link, String key, Class<T> clazz) {
        try {
            return new ObjectMapper().readValue(new JsonApi(link + key).executeRequest().asString(), clazz);
        } catch (Exception e) {
            return null;
        }
    }
